...
restart_quiz.onclick = () => {
  ...
  queCounter(que_number); //passing que_number value to queCounter
...
next_btn.onclick = () => {
  if (que_count < questions.length - 1) {
    ...
    que_number++; //increment the que_number value
    showQuetions(que_count); //calling showQestions function
    queCounter(que_number); //passing que_number value to queCounter
    clearInterval(counter); //clear counter
    clearInterval(counterLine); //clear counterLine
...
function showResult() {
  ...
  if (userScore > 5) {
    ...
    let scoreTag =
      "<span>լավ չէ, բայց ունեք դեռ շատ աշխատելու տեղ 😎, Դուք հավաքեցիք <p>" +
      userScore +
      "</p> - <p>" +
      questions.length +
      "</p>ից</span>";
